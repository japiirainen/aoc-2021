      ...
      then Literal <$> parseLiteral
        ...
        case lengthType of
          False -> do
            ...
            op ty =<< parseUntil numBits parsePacket
          True -> do
            len <- toInt <$> parseBits 11
            op ty =<< replicateM len parsePacket
  where
...
versionSum = cata $ \(Packet v _ vs) -> v + sum vs
...
main = pureMain $ \input -> do
  packet <- runParser parsePacket $ fromHexString input
