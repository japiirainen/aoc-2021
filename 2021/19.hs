...
import AOC.List.Extended (select)
import AOC.Main (pureMain)
import AOC.Parsija as P
import AOC.V3 (V3 (..), manhattan, (.+.), (.-.))
...
orientations =
  let vars axis b = [b, b <> [axis], b <> [axis, axis], b <> [axis, axis, axis]]
   in vars RotZ []
        <> vars RotX [RotY]
        <> vars RotZ [RotY, RotY]
...
parseScan =
  Scanner
    <$> (P.string "--- scanner " *> P.decimal <* P.spaces <* P.string "---" <* P.spaces)
    <*> (S.fromList <$> parseV3 `P.sepBy1` P.spaces)
  where
    parseV3 =
      V3
...
main = pureMain $ \input -> do
  scanners <- P.runParser (parseScan `P.sepBy1` P.spaces) input
  Scan bs ss <-
    maybe (Left "Unable to merge all scanners") Right $
