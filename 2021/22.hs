...
intersection (Box lo0 hi0) (Box lo1 hi1)
  ...
  | otherwise =
    Just $
      Box
        (V3.zipWith max lo0 lo1)
        (V3.zipWith min hi0 hi1)
...
dumb bounds boxes = VU.length $ VU.filter id vec
  where
    ...
    idx v =
      let (V3 x y z) = v .-. bMin bounds
       in x + width bounds * (y + depth bounds * z)
...
smart =
  ...
  where
    ...
    remove bs box =
      [(i, not on) | (b, on) <- bs, i <- maybeToList $ intersection b box] <> bs
    toVolume (b, on) = (if on then id else negate) . volume $ fromIntegral <$> b

parseBox :: String -> (Bool, Box Int)
parseBox line =
  let state = "on" `isPrefixOf` line
      clear c = if isDigit c || c == '-' then c else ' '
   in case map read . words $ map clear line of
        [x0, x1, y0, y1, z0, z1] ->
...
main = simpleMain $ \input ->
  let boxes = map parseBox $ lines input
      bounds = Box (V3 (-50) (-50) (-50)) (V3 50 50 50)
   in (dumb bounds boxes, smart boxes)
