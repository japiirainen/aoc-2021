...
import Control.Lens hiding (index, noneOf)
...
import System.Exit
import Text.Megaparsec hiding (Pos, State (..), choice, many, some, takeP, takeWhile1P, takeWhileP)
import Text.Megaparsec qualified
import Text.Megaparsec.Char
import Text.Megaparsec.Char.Lexer (binary, decimal, hexadecimal, octal, signed)
...
parseE :: Parser a -> String -> Either String a
parseE p s = case runParser (p <* eof) "" s of
...
takeP :: MonadParsec e s m => Int -> m (Tokens s)
takeP = Text.Megaparsec.takeP Nothing

takeWhileP :: MonadParsec e s m => (Token s -> Bool) -> m (Tokens s)
...
howManyOf :: Num a => Getting (Endo (Endo a)) s t -> (t -> Bool) -> s -> a
...
findDuplicatesBy f = go Set.empty
  where
    go seen (x : xs)
      | r `Set.member` seen = x : xs'
      | otherwise = xs'
      where
        xs' = go (Set.insert r seen) xs
        r = f x
    go _ [] = []
...
dimensions g = (xmax - xmin + 1, ymax - ymin + 1)
  where
    ((xmin, ymin), (xmax, ymax)) = boundingBox g

rotateGrid :: GridLike a => a -> a
rotateGrid g = mapCoords (\(x, y) -> (y, width - x - 1)) g
  where
...
evolve neighbours rule alive = MapS.keysSet (MapS.filter rule cells)
  where
    cells = MapS.fromListWith (<>) (foldMap neighbourhood alive)
    neighbourhood p =
      [ (p', Cell self (1 - fromEnum self))
        | p' <- neighbours p,
          let self = p == p'
      ]
...
dfsOn rep next start = go Set.empty (Seq.singleton (start, 0))
  where
    go _ Seq.Empty = []
    go seen (ps Seq.:|> (n, d))
...
bfsOn rep next start = go Set.empty (Seq.singleton (start, 0))
  where
    go _ Seq.Empty = []
    go seen ((n, d) Seq.:<| ps)
      | r `Set.member` seen = go seen ps
...
dijkstraOn rep next start = go Set.empty (PQ.singleton 0 start)
  where
    go seen q
      | Just ((d, n), q') <- PQ.minViewWithKey q =
        let r = rep n
         in if r `Set.member` seen
              then go seen q'
              else (n, d) : go (Set.insert r seen) (PQ.union q' (PQ.fromList [(d + c, n') | (n', c) <- next n]))
      | otherwise = []
