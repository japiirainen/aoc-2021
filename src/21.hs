...
playGame dice player1 player2 =
    ...
    else playGame dice' player2 player1'
...
wins pos1 pos2 = fold $ M.intersectionWith
    (\pts1 pts2 -> mconcat $ do
        ...
        -- Player 2 does need to reach this turn, so remove possiblities
        let _ = div c2 . sum $ map snd dice3
        pure $
...
main = simpleMain $ \input -> do
    let [pos1, pos2] = map (read . last . words) $ lines input
        ...
        part1 = diceRolls dice * playerScore loser
        part2 = maximum $ getSum <$> wins pos1 pos2 in
     (part1, part2)
